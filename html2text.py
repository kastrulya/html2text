...
class HTML2Text(HTMLParser.HTMLParser):
    ...
    def handle_tag(self, tag, attrs, start):
        ...
        if tag in ['em', 'i', 'u'] and not self.ignore_emphasis: self.o(
            self.emphasis_mark)
        ...
        if start:
            if not self.parent_tag:
                self.parent_tag = "p" if tag == "p" else self.parent_tag
        else:
        ...
        if tag == 'li':
            ...
            if start:
                ...
                self.o(
                    "  " * nest_count)  # TODO: line up <ol><li>s > 9 correctly.
                if li['name'] == "ul":
    ...
    def o(self, data, puredata=0, force=0):
        ...
        if not self.quiet:
            ...
            bq = (">" * self.blockquote)
            if not (force and data and data[
                0] == ">") and self.blockquote: bq += " "
            ...
            if self.a and ((
                ...
                for link in self.a:
                    if self.outcount > link['outcount']:
                        self.out("   [" + str(
                            link['count']) + "]: " + urlparse.urljoin(
                            self.baseurl, link['href']))
                        if has_key(link, 'title'): self.out(
                            " (" + link['title'] + ")")
                        self.out("\n")
                    else:
                        newa.append(link)
    ...
    def google_nest_count(self, style):
        ...
        if 'margin-left' in style:
            nest_count = int(
                style['margin-left'][:-2]) / self.google_list_indent
        return nest_count
